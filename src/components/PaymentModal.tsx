    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg border border-blue-100 dark:border-blue-800 shadow-sm">
          ...
          <div className="flex border-b border-gray-200 dark:border-gray-700 mb-4 w-full overflow-x-auto pb-2">
            {bankDetails.map((bank) => (
              <button
                key={bank.id}
                onClick={() => setSelectedBank(bank.id)}
                className={`py-2 px-2 sm:px-4 flex-1 text-sm font-medium flex flex-col items-center min-w-[80px] sm:min-w-0 ${selectedBank === bank.id
                  ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400'
                  : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}
              >
                <div className="w-5 h-5 mb-1 bg-white rounded-md flex items-center justify-center overflow-hidden">
                  <img
                    src={bank.logo_path}
                    alt=""
